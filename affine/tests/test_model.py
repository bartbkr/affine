...
class TestInitiatilize(TestCase):
    """
    Tests for methods related to inialization of a new Affine object
    """
    def setUp(self):

        #initialize yield curve and VAR observed factors
        yc_data_test = pa.DataFrame(np.random.random((test_size - lags, nyields)))
        ...
        #This sets up a fake model where only lambda_0 and lambda_1 are estimated
