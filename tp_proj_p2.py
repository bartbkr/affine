...
irsf = vreg.irf(periods=30)
#irsf.plot(orth=True, stderr_type='mc', impulse='tr_empl_gap', repl=1000)
#plt.show()
...
#for t in range(k_ar, len(mod_data)):
    ...
    #for i in range(120):
        #for p in range(neqs):
            ...
            #new_dat[0,p] = params[0,p] + \
                    #np.dot(np.flipud(s_data)[:k_ar].flatten()[None], \
                    #params[1:,p,None])
...
mod_yc_data = ycdata.reindex(columns=['l_tr_m3', 'l_tr_m6',
                                      'l_tr_y1', 'l_tr_y2',
                                      'l_tr_y3', 'l_tr_y5',
                                      'l_tr_y7', 'l_tr_y10']).dropna(axis=0)
...
#plot 10_mth - 3_mth
t_struct_move = px.DataFrame(index=mod_yc_data.index)
t_struct_move['ten_min_3m'] = mod_yc_data['l_tr_y10'] - mod_yc_data['l_tr_m3']
t_struct_move.plot()


"""
##############################################
# Attempt to solve Ang and Piazzesi model    #
##############################################

mod_data_ap = mthdata.reindex(columns=['tr_empl_gap_perc',
                                    'act_infl']).dropna(axis=0)
mod_ap = VAR(mod_data_ap, freq='M')
vreg_ap = mod_ap.fit(maxlags=4)
k_ar_ap = vreg_ap.k_ar
neqs_ap = vreg_ap.neqs
lat_ap = 3

#create BSR X_t
x_t_na = mod_data_ap.copy()
for t in range(k_ar_ap-1):
    for var in mod_data_ap.columns:
        x_t_na[var + '_m' + str(t+1)] = px.Series(mod_data[var].values[:-(t+1)],
                                            index=mod_data.index[t+1:])
#remove missing values
X_t_ap = x_t_na.dropna(axis=0)
mod_yc_data_ap = ycdata.reindex(columns=['l_tr_m3', 'l_tr_m6',
                                      'l_tr_y1', 'l_tr_y2',
                                      'l_tr_y3', 'l_tr_y5',
                                      'l_tr_y7', 'l_tr_y10']).dropna(axis=0)
mod_yc_data_ap = mod_yc_data.join(X_t_ap['act_infl'], how='right')
mod_yc_data_ap = mod_yc_data_ap.drop(['act_infl'], axis=1)
rf_rate = px.DataFrame(mthdata['fed_funds'].dropna())
rf_rate = rf_rate.join(X_t_ap['act_infl'], how='right')
rf_rate = rf_rate.drop(['act_infl'], axis=1)
mod_data_ap = mod_data_ap.join(rf_rate['fed_funds'], how='right')
mod_data_ap = mod_data_ap.drop(['fed_funds'], axis=1)

#angpia = BSR(yc_data = mod_yc_data_ap[3:], var_data=mod_data_ap,
#        rf_rate=rf_rate, maxlags=4, latent=3, no_err = [3, 36, 84])

#guesses
np.random.seed(100)
#lam_0_gr = [0.0]*(neqs_ap + lat_ap)
lam_0_gr = np.random.randn(neqs_ap + lat_ap,1)[:,0].tolist()
#lam_1_gr = [0.0]*((neqs_ap + lat_ap)**2)
lam_1_gr = np.random.randn((neqs_ap + lat_ap)**2,1)[:,0].tolist()
#delt_1_gr = [0.0]*lat_ap
delt_1_gr = np.random.randn(lat_ap,1)[:,0].tolist()
#phi_gr = [0.0]*(lat_ap**2)
phi_gr = np.random.randn(lat_ap**2,1)[:,0].tolist()
#sig_gr = [0.0]*(lat_ap**2)
sig_gr = np.random.randn(lat_ap**2,1)[:,0].tolist()

ap_solv = angpia.solve(lam_0_gr, lam_1_gr, delt_1_g = delt_1_gr,
                       phi_g = phi_gr, sig_g = sig_gr, xtol=1e-100)
...
#print 'AIC'
#print VAR(mod_data_ap, freq='M').fit(maxlags=20, ic='aic').k_ar
#print 'FPE'
#print VAR(mod_data_ap, freq='M').fit(maxlags=20, ic='fpe').k_ar
#print 'HQIC'
#print VAR(mod_data_ap, freq='M').fit(maxlags=20, ic='hqic').k_ar
#print 'BIC'
#print VAR(mod_data_ap, freq='M').fit(maxlags=20, ic='bic').k_ar
